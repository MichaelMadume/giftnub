 ...
 * This prevents duplication and ensures consistency between frontend and backend
 */

import { shuffle } from 'lodash';

// Import zod for schema validation (needs to be added to dependencies)
import { z } from 'zod';
...
// Zod schema for GiftSuggestion
export const GiftSuggestionSchema = z.object({
  id: z.string().describe("Unique identifier for the suggestion"),
  title: z.string().describe("Marketing title for the gift suggestion"),
  description: z.string().describe("Enticing marketing message telling the user what could possibly be offered"),
  giftId: z.string().optional().describe("Reference to the most similar and relevant gift from the catalog if that exists")
});

// TypeScript type for the gift suggestion
export type GiftSuggestion = z.infer<typeof GiftSuggestionSchema>;

// Zod schema for structured AI response
export const GiftSuggestionResponseSchema = z.object({
  marketingTheme: z.string().describe("Theme of gifting, a concise theme for the gifting experience (4 words recommended)"),
  marketingMessageSummary: z.string().describe("Overall warm message telling the user what the company can do for them very briefly"),
  suggestions: z.array(GiftSuggestionSchema).describe("List of possible gift suggestions")
});

// TypeScript type for the structured response
export type GiftSuggestionResponse = z.infer<typeof GiftSuggestionResponseSchema>;
